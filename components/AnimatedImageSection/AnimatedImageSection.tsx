...
const AnimatedImageSection = () =>  {
  ...
  useEffect(() => {
    ...
    // Animate Lines SVG - Draw from left-top corner using clip-path on container
    if (linesRef.current) {
      // Create timeline for smooth diagonal reveal
      const tl = gsap.timeline();
      
      // Start from a point at top-left
      gsap.set(linesRef.current, {
        clipPath: 'polygon(0 0, 0 0, 0 0)',
      });

      // First expand horizontally from left
      tl.to(linesRef.current, {
        clipPath: 'polygon(0 0, 100% 0, 100% 0, 0 0)',
        duration: 0.8,
        ease: 'power2.out',
      })
      // Then expand vertically to reveal the full shape
      .to(linesRef.current, {
    ...
    // Animate Dots SVG - Scale in step by step
    if (dotsRef.current) {
  ...
  return (
    <div ref={containerRef} className="relative flex-shrink-0">
      {/* Lines SVG - Behind, left-top corner */}
      <div ref={linesRef} className="absolute -left-4 top-20 z-0 opacity-80">
      ...
      {/* Dots SVG - Ahead, right-bottom corner */}
      <div className="absolute -right-4 -bottom-4 z-20 opacity-90">
        <svg 
          ref={dotsRef}
          width="84" 
          height="84" 
